...
async function scrapeInstagram(username: string) {
  ...
  const res = await fetch(
    ...
    { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ usernames: [username] }), cache: 'no-store' }
  )
  if (!res.ok) throw new Error(`Apify ${res.status} for @${username}`)
  const items = await res.json()
  const profile = items?.[0]
  if (!profile) throw new Error(`No data for @${username}`)
